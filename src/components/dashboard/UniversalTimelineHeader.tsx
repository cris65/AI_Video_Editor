...
interface UniversalTimelineHeaderProps {
  ...
  // Keyboard Shortcuts + Markers state
...
export const UniversalTimelineHeader: React.FC<UniversalTimelineHeaderProps> = ({
  ...
  return (
    <div className="flex justify-between items-center mb-2 px-2 relative">
      ...
      {/* Mode-specific tools (CENTERED) */}
      <div className="absolute left-1/2 -translate-x-1/2 flex items-center gap-2 z-[100]">
        {mode === 'stringout' && (
        ...
        {/* Unified Marker & Navigation Control Panel */}
        ...
        {mode === 'director_cut' && dcActions && (
          <div className="flex items-center gap-2 ml-2 border-l border-slate-700 pl-4 relative z-[100]">
            <button onClick={dcActions.onBookendStart} className="px-2 py-1 text-[10px] font-bold bg-blue-900/30 text-blue-400 hover:bg-blue-800/50 rounded transition-colors border border-blue-900/50">
              [ IN
            </button>
            <button onClick={dcActions.onBookendEnd} className="px-2 py-1 text-[10px] font-bold bg-purple-900/30 text-purple-400 hover:bg-purple-800/50 rounded transition-colors border border-purple-900/50">
              OUT ]
            </button>
          </div>
        )}
