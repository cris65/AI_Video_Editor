...
5. EXECUTE: Run this exact chained command in the shell:
   `git add . && git commit -m "<your_generated_message>" && git push origin develop && mkdir -p ./WOLF_EXPORTS && cp .gemini/*.md ./WOLF_EXPORTS/ && cp GEMINI.md ./WOLF_EXPORTS/ && echo '✅ WOLF_EXPORTS Export Completed!' && afplay /System/Library/Sounds/Glass.aiff`
