...
- **Audio Rhythm Engine & Dual Waveform:** Estrazione avanzata delle tracce audio (Percussive/Harmonic, Beats, BPM) tramite `librosa` (supporta `.wav`, `.m4a`, `.aac`, `.flac`). La UI della modale Audio è *pre-idratata* con i dati di sessione, mostrando immediatamente lo stato "Ready for Processing". Overlay d'onda verde generato come SVG puro, con clamping esatto della larghezza alla durata reale dell'audio per evitare distorsioni grafiche sulla timeline. Presente anche un avviso intelligente con checkbox per sincronizzare automaticamente la `target_duration` dell'intero progetto alla lunghezza della traccia musicale appena scansionata. In modalità "Waveform Control" l'onda è switchabile tra **Amplitude** ed **Energy**.
