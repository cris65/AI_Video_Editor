...
6. **`director.py`** — Fase D. AI Director ragionante. Riceve la lista clip con i dati semantici, interpella il modello LLM selezionato per una `editing_recipe`, poi applica la recipe su una griglia matematica di beat. Gestisce il sistema Pillar/Filler e il Safety Net auto-fill. Output: `_final_edit.json` (file di lavoro corrente, sovrascrivibile) + `_gemma_recipe.json`. **History Archive System:** dopo ogni inferenza, salva una copia immutabile e arricchita in `engine/output/{sequence_name}/history/` con nomenclatura `[TIMESTAMP]_[MODEL]_[DURATION-MODE]_recipe.json`. Il blocco `_metadata` include: `timestamp`, `brain_model`, `inference_time_seconds` (misurato con `time.perf_counter()`), `user_directives` (Strictness, Energy, Target Duration, Style Prompt), `director_vision`, `clip_count`.
...
- **Audio Rhythm Engine & Dual Waveform:** Estrazione avanzata delle tracce audio (Percussive/Harmonic, Beats, BPM). Overlay d'onda verde generato come SVG puro dinamicamente switchabile via "Waveform Control" tra **Amplitude** (volume grezzo, picchi Premiere-style) ed **Energy** (tensione ritmica da librosa onset envelope). Griglia dei Beat/Metronomo proiettata verticalmente sull'onda sonora e bypassabile via Audio Marker Filters.
