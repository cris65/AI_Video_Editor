...
9. **`api_server.py`** — Runtime server FastAPI (:8000). Espone `/api/system/profiler` (rilevamento hardware Apple Silicon), `/api/orchestrate` (endpoint POST che riceve il payload ibrido dalla UI e attiva la Fase D isolata), e i nuovi endpoint dell'Audio Rhythm Engine (`/api/audio/files`, `/api/audio/analyze`). Il modello Pydantic `DirectorConfigPayload` accetta i campi di Fase 2: `duration_mode`, `rhythmic_strictness`, `energy_threshold`, `audio_marker_priority`, `seed`, `ai_model`. Il modello `UserConstraint` accetta i tipi `IN`, `OUT`, `BM` e `AUDIO`. Il modello `LockedClipOverride` accetta i campi del Bookend System: `force_status`, `is_global_start`, `is_global_end`, `bookend_start_time`, `bookend_end_time`.
...
- **Audio Rhythm Engine & Dual Waveform:** Estrazione avanzata delle tracce audio (Percussive/Harmonic, Beats, BPM) tramite `librosa` (supporta `.wav`, `.m4a`, `.aac`, `.flac`). La UI della modale Audio è *pre-idratata* con i dati di sessione, mostrando immediatamente lo stato "Ready for Processing". L'overlay d'onda verde è generato come SVG puro, applicato identicamente sia su `InteractiveTimeline` che `FinalCutTimeline`, con calcolo percentuale `waveformWidthPct` proporzionale limitato a `Math.min(duration, audioDuration)` per interrompere graficamente la traccia al millisecondo esatto di fine. Presente anche un avviso intelligente con checkbox a "Theme Dinamico" (Ambra = Warning / Verde = Sincronizzato) per forzare o sganciare bidirezionalmente la `target_duration` (con fallback a 60s). In modalità "Waveform Control" l'onda è switchabile tra **Amplitude** ed **Energy**.
...
- **Sistema Multi-Anchor (BM/IN/OUT/AUDIO) + Bookend globali:** Vincoli multipli per clip via shortcut (`M`, `I`, `O`, `A`), rimovibili chirurgicamente via `X`. I Bookend globali (`[ IN` / `OUT ]`) utilizzano `clipOverrides` esteso con i campi `is_global_start`, `is_global_end`, `bookend_start_time`, `bookend_end_time`, posizionando il marcatore al timestamp esatto della playhead (non al bordo della clip). Tutti e 6 i tipi di marker sono visualizzati nella ClipCard con la palette colori Premiere Pro: IN=`#4CAF50`, OUT=`#E53935`, BM=`#FF6D00`, AUDIO=`#FFC107`, Bookend IN=`#3B82F6`, Bookend OUT=`#A855F7`.
- **ClipCard Layout 4-Aree:** Area A (top bar thumbnail): nome clip | tag MAIN_A | Actors. Area B (centro thumbnail): badge Bookend `[ IN` / `OUT ]`. Area C (bottom bar thumbnail): `🔥 N MARKERS SET` (sinistra) + timer durata (destra), conteggio include tutti i tipi. Area D (prima riga content): pallini analisi cromatica. Score MLX sotto di essa.
...
- **Director Settings Panel & Orchestrazione:** Sidebar rapida che invia l'intero stato ibrido (Seed, Constraints, Overrides) all'endpoint FastAPI `/api/orchestrate`. Creative Settings Portal (`AdvancedDirectorModal`) full-screen per prompt e parametri NLP. Include visualizzatori vettoriali (SVG) per la `Rhythmic Strictness` (onde di Bezier + clip tolleranza) e la `Minimum Energy Threshold` agganciata nativamente all'array `audioBeats` reale esportato dall'engine. Il campo `Analysis Rate (FPS)` è stato rimosso dalla Fase 2 (appartiene esclusivamente alla Fase 1 — `PhaseAPayload`). Il selettore modello AI espone 3 opzioni: `gemma-4-4b` → `mlx-community/gemma-4-e4b-it-4bit`, `gemma-4-31b` → `mlx-community/gemma-4-31b-it-4bit`, `llama-3.3-70b` → `mlx-community/Llama-3.3-70B-Instruct-4bit`. La risoluzione avviene in `director.py` tramite `AI_MODEL_MAP` con priorità UI su metadati stringout.
...
- **Keyboard Shortcuts Panel:** Pannello popup color-coded con palette Premiere per tutti e 4 i tipi di marker (IN verde, OUT rosso, BM arancio, Audio gold). Legenda footer timeline completa con 8 item (Valid, B-ROLL, Trash + 4 marker types + 2 bookends).
