...
5. EXECUTE: Run these exact sequence of commands in the shell:
   ```bash
   git add .
   git commit -m "<your_generated_message>"
   rm -rf WOLF_EXPORTS && mkdir -p WOLF_EXPORTS && cp .gemini/*.md WOLF_EXPORTS/ && cp GEMINI.md WOLF_EXPORTS/
   echo '✅ WOLF_KB Export Completed!' && afplay /System/Library/Sounds/Glass.aiff
   ```
...
7. FINISH: Add a final log message to the user: "WOLF-FLOW COMPLETED. Local commit and KB export done. Please run `git push origin develop --no-verify` to deploy."
